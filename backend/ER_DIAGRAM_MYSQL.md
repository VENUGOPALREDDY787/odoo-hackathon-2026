...
users (1) ──────< (0..1) customers
    ...
    │                    └── user_id → users (customer portal login)
    └── role: ENUM('rep','manager','finance','admin','customer')
```

**Cardinality:**
- One user can be linked to at most one customer record (as portal user via `user_id`)
- One customer has exactly one assigned rep (`users.role = 'rep'`) via `assigned_rep_id`
- One customer has zero or one primary contact (`users.role = 'customer'`) via `primary_contact_id`
- All FKs: `ON DELETE SET NULL` (preserve customer if user deleted)
...
**Cardinality:**
- Category hierarchy: unlimited depth via `parent_id` self-reference (`ON DELETE RESTRICT`)
- One category contains many products (`ON DELETE RESTRICT` - prevent orphaned products)
- One product has many variants (size, color, config) (`ON DELETE CASCADE` - variants die with product)
...
- Discount tiers: M:N resolved via two nullable FKs with CHECK constraint
  - Either `category_id` OR `product_id` is set (never both) - enforced by `chk_discount_either`
- Price list items: unique per (price_list, product, variant, tier, qty_range) via composite index
...
**Cardinality:**
- One approval chain defines rules for a discount range (`min_discount_percent` to `max_discount_percent`)
- `required_approver_roles` stored as JSON array of ENUM values
- One quotation can have many approval log entries (workflow steps)
- Approval chain: `min_approvals_required` + role array defines quorum
...
customers (1) ────────────────< (0..N) quotations
    ...
    │                              └── status: ENUM('draft','pending_approval','approved','rejected','sent','accepted','expired','cancelled')
    │
quotations (1) ─────────────────< (0..N) quotation_lines
    ...
    │                                   └── line_type: ENUM('one_time','recurring')
    │
quotation_lines (1) ──────────────< (0..N) fulfillment_splits
    │                                   │
    │                                   ├── warehouse_id → warehouses (RESTRICT)
    │                                   └── status: ENUM('pending','partial','shipped','delivered','cancelled')
    │
...
- **Generated columns** for computed pricing: `net_unit_price`, `line_subtotal`, `tax_amount`, `line_total`
- **Stock reservation**: `quantity_available` is STORED generated column (`quantity_on_hand - quantity_reserved`)
...
**Cardinality:**
- Stock levels track `on_hand`, `reserved`, `available` (computed STORED)
- Fulfillment splits reference warehouse directly for shipping
- Row-level locking on `stock_levels` enables atomic reservation: `UPDATE ... WHERE quantity_available >= X`
...
subscription_plans (1) ────────< (0..N) quotation_lines (recurring)
    ...
    │                                └── subscription_plan_id → subscription_plans (SET NULL)
    │
    └── interval_type: ENUM('monthly','quarterly','yearly')
        proration_rule: ENUM('none','full','partial','day_based')
```

**Cardinality:**
- Plans define billing interval, pricing, trial, proration
- Quotation lines reference plan for recurring revenue
- `billing_schedules` materialize each billing cycle per line
...
upsell_rules (1) ───────────────> trigger: product_id XOR category_id
    ...
    │
    └── condition_type: ENUM('always','quantity_threshold','customer_tier','custom')
```

---

### 9. Negotiation & Deal Health
```
quotations (1) ────────────────< (0..N) negotiation_logs
    │                                │
    │                                ├── initiated_by → users (RESTRICT)
    │                                ├── resolved_by → users (SET NULL)
    │                                └── status: ENUM('active','countered','accepted','rejected','expired')
    │
...
**Cardinality:**
- Negotiation logs track full back-and-forth history with JSON snapshots
...
- Partitioned by month for performance (MySQL 8.0+ native partitioning)
- Never deleted (no `deleted_at` column) - immutable audit log
- JSON columns for flexible old/new values storage
...
| stock_levels | quantity_available (GENERATED STORED) | Frequent filter in availability checks; stored for index efficiency |
| quotation_lines | net_unit_price, line_subtotal, tax_amount, line_total (GENERATED STORED) | Line totals used in quotation totals, PDF generation, and reporting |
...
- Generated STORED columns where purely computational
- Triggers/stored procedures for cross-table aggregates (quotation totals)
...
| Product SKU lookup | `uk_products_sku, uk_product_variants_sku` |
| Quotation number lookup | `uk_quotations_number` |
| Period-based reporting | `idx_quotations_date_range`, `idx_billing_schedules_period` |

**Partial/Filtered Indexes** (MySQL 8.0+):
- `WHERE deleted_at IS NULL` on all soft-delete tables
- `WHERE is_active = 1` on reference tables
- `WHERE is_acknowledged = 0` on alerts
...
Application queries MUST include `WHERE deleted_at IS NULL` (can be enforced via views).

Hard deletes are NEVER used - `audit_trails` preserves full history with `SOFT_DELETE` operation type.
...
|---------|----------------|
| **ENUM types** | Native `ENUM('val1','val2')` for role, tier, status - enforced at storage layer |
| **UUID PK** | `CHAR(36)` with `DEFAULT (UUID())` - application can also provide |
| **DATETIME** | Used for all timestamps (no 2038 limit like TIMESTAMP) |
| **ON UPDATE** | `updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP` |
| **Generated columns** | STORED for `quantity_available`, pricing computations - indexable |
| **JSON** | Native JSON type for addresses, metadata, audit values, role arrays |
| **CHECK constraints** | Enforced in MySQL 8.0+ for ranges, XOR logic, enum validation |
| **Partitioning** | `audit_trails` by RANGE on `created_at` (monthly) |
| **Row-level locking** | `SELECT ... FOR UPDATE` on `stock_levels` for atomic reservation |