...
CREATE TABLE `quotations` (
    ...
    -- Margin & Versioning for Optimistic Locking
    `version` INT NOT NULL DEFAULT 1,
    `margin_total` DECIMAL(14,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE `billing_schedules` (
    ...
    `currency` CHAR(3) NOT NULL DEFAULT 'USD',
    `status` ENUM('pending','invoiced','paid','overdue','cancelled','credit_note') NOT NULL DEFAULT 'pending',
...
CREATE TABLE `idempotency_keys` (
    ...
    `expires_at` DATETIME NOT NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_idempotency_key_path` (`key`, `request_path`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
